 ...
 * 売上情報用のDAO
 */
public class SalesDAO extends DBConnection {
    ...
    public List<Sales> selectByUserId(String userId) throws Exception {
        ...
        String sql = "  SELECT                                       "
                + "   ts.sales_id,                                " + "   ts.user_id,                                 "
                + "   mu.user_name,                               " + "   ts.product_id,                              "
                + "   mp.product_name,                            " + "   mp.price,                                   "
                + "   ts.quantity,                                " + "   ts.sales_date                               "
                + " FROM                                          " + "   t_sales ts                                  "
                + "   INNER JOIN m_product mp                     " + "           ON ts.product_id = mp.product_id    "
                + "   LEFT OUTER JOIN m_user mu                   " + "                ON ts.user_id = mu.user_id     "
                + " WHERE                                         " + "   ts.user_id = ?                              "
                + " ORDER BY                                      " + "   ts.sales_date DESC,                         "
                + "   ts.sales_id ASC                             ";
        ...
        try (Connection con = getConnection(); PreparedStatement pstmt = con.prepareStatement(sql)) {
            ...
            try (ResultSet res = pstmt.executeQuery()) {
                while (res.next()) {
                    ...
                    Sales sales = new Sales(salesId, userId, userName, productId, productName, price, quantity,
                            salesDate);
                    salesList.add(sales);
                }
            }
